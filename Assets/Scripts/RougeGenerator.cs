...
public class RougeGenerator
{
    ...
    const int MAX_ROOM_SIZE = 40;
    const int MIN_SPACE_BETWEEN_ROOM_AND_ROAD = 2;
    int width, height;
    bool isOpenTop, isOpenLeft, isOpenRight, isOpenBottom;
    int[,] map;
    List<Area> areaList;

    public RougeGenerator(MapBase mapBase)
    {
        this.width = mapBase.MapWidth;
        this.height = mapBase.MapHeight;

        this.isOpenTop = mapBase.OpenTop;
        this.isOpenLeft = mapBase.OpenLeft;
        this.isOpenRight = mapBase.OpenRight;
        this.isOpenBottom = mapBase.OpenBottom;
    ...
    void CreateRoomInArea(Area area)
    {
        ...
        for (int y = roomTop; y < roomBottom; y++)
        {
            ...
            {
                if (x == buildPointX && y == buildPointY)
                {
    ...
    void CreateEntry()
    {
        int layerType = (int)CellType.Layer;
        int groundType = (int)CellType.Ground;
        int entryType = (int)CellType.Entry;
        int areaLineType = (int)CellType.AreaLine;
        ...
        for (int y = 0; y < height; y++)
        {
            for (int x = 0; x < width; x++)
            {
                if (map[x, y] != layerType && map[x, y] != areaLineType)
                {
                    xLeft = x;
                    yTop = y;
                }
        ...
        if (isOpenTop)
        {
            int entryPoint = Random.Range(xLeft, xRight);
            map[entryPoint, 0] = entryType;
            for (int y = 1; y < height; y++)
            {
                if (map[entryPoint, y] == layerType || map[entryPoint, y] == areaLineType)
                {
                    map[entryPoint, y] = groundType;
                }
                else
                {
                    break;
        ...
        if (isOpenLeft)
        {
            int entryPoint = Random.Range(yTop, yBottom);
            map[0, entryPoint] = entryType;
            for (int x = 1; x < width; x++)
            {
                if (map[x, entryPoint] == layerType || map[x, entryPoint] == areaLineType)
                {
                    map[x, entryPoint] = groundType;
                }
                else
                {
                    break;
        ...
        {
            int entryPoint = Random.Range(yTop, yBottom);
            map[width - 1, entryPoint] = entryType;
        ...
        {
            int entryPoint = Random.Range(xLeft, xRight);
            map[entryPoint, height - 1] = entryType;
            for (int y = height - 2; 0 < y; y--)
            {
                if (map[entryPoint, y] == layerType || map[entryPoint, y] == areaLineType)
                {
                    map[entryPoint, y] = groundType;
    ...
    void CreateWall()
    {
        ...
        for (int y = 0; y < height; y++)
        {
            for (int x = 0; x < width; x++)
            {
                if (map[x, y] == layerType)
                {
                    int targetX = (x - 1 < 0 || x + 1 >= width) ? 0 : 1;
                    int targetY = (y - 1 < 0 || y + 1 >= height) ? 0 : 1;

                    bool isWall = false;
                    isWall = isWall || ((map[x + targetX, y] != layerType && map[x + targetX, y] != wallType)) ? true : false;
                    isWall = isWall || ((map[x + targetX, y + targetY] != layerType) && (map[x + targetX, y + targetY] != wallType)) ? true : false;
                    isWall = isWall || ((map[x + targetX, y - targetY] != layerType) && (map[x + targetX, y - targetY] != wallType)) ? true : false;
                    isWall = isWall || ((map[x - targetX, y] != layerType && map[x - targetX, y] != wallType)) ? true : false;
                    isWall = isWall || ((map[x - targetX, y + targetY] != layerType) && (map[x - targetX, y + targetY] != wallType)) ? true : false;
                    isWall = isWall || ((map[x - targetX, y - targetY] != layerType) && (map[x - targetX, y - targetY] != wallType)) ? true : false;
    ...
    void DrawRoadFromRoomToConnectLine(Area area)
    {
        for (int y = 0; y < area.Road.Height; y++)
        {
            ...
            {
                map[x + area.Road.Left, y + area.Road.Top] = (int)CellType.Ground;
            }
        }
    }
